...
class ArticleWeekHotCommentViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun getWeekHotCommentNews(refreshStatus: Int) {
        ...
        AwakerRepository.get().getHotNewsAll(ConstantUtils.TOKEN, page, 0)
                .observeOn(AndroidSchedulers.mainThread())
                .doOnError {}
                .doOnSubscribe {
                ...
                .doOnNext { newsList ->
                    ...
                    newsList?.let { setWeekHotCommentNewsLocalDb(newsList) }
                }
                .subscribe(Subscriber.create())
    }

    private fun setWeekHotCommentNewsLocalDb(localNewsList: List<News>) {
        Flowable.create<Any>({ e ->
            val newsEntity = NewsEntity(NewsEntity.HOT_NEWS_ALL,
                    localNewsList)
